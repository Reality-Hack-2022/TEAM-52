...
namespace LookingGlass.Editor {
    public static class Preview {
        ...
        private static void OpenAllHoloplayWindows() {
            ...
            //NOTE: If we don't want to update user game views' resolution(s), just comment out this method call below:
            SetUserGameViewResolutions();
        }
        ...
        public static bool UpdatePreview() => UpdatePreview(true);
        private static bool UpdatePreview(bool forceReopenToUpdate) {
            if (!IsActive)
                return false;

            if (forceReopenToUpdate) {
                CloseAllHoloplayWindows();
                OpenAllHoloplayWindows();
            }
        ...
        /// <summary>
        /// Updates any user's game view(s) that are NOT editor preview windows, to the resolution of <see cref="Holoplay.Instance"/>.
        /// This is done for the user's convenience, so that their game view resolutions are kept in-sync with the native resolution of the latest Holoplay that was enabled.
        /// </summary>
        private static void SetUserGameViewResolutions() {
            ...
            if (main != null) {
                ...
                foreach (EditorWindow gameView in GameViewExtensions.FindUnpairedGameViews())
                    gameView.SetGameViewResolution(cal.screenWidth, cal.screenHeight, main.DeviceTypeName);
            }
        }
    }
}
